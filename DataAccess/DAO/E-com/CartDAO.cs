            ...
            try
            {
                using (var context = new AppDbContext())
                {
                    var queryResult = from c in context.Carts
                                      ...
                                      join i in context.Inventories on b.ProductId equals i.ProductId
        ...
        public int AddProductToCart(Guid productId, Guid cartId, int quantity)
        {
            try
            {
                ...
                using (var context = new AppDbContext())
                {
                    ...
                    {
                        string insertQuery = $"INSERT INTO Baskets (BasketId, ProductId, CartId, OrderId, Quantity, AddedDate, Stored_Price) " +
                     $"VALUES (@basketId, @productId, @cartId, NULL, @quantity, @addedDate, NULL)";

                        result = context.Database.ExecuteSqlRaw(insertQuery,
                            new SqlParameter("@basketId", Guid.NewGuid()),
